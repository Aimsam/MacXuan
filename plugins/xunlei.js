...
TLE.exporter = {
  ...
  "麦旋风下载": function(todown) {
    if (TLE.getConfig("TLE_aria2_jsonrpc")) {
      show_tip("添加中...到麦旋风下载管理中查看是否添加成功");
...
(function(TLE) {
  ...
  TLE.getConfig = function(key) {
    if (key == 'TLE_aria2_jsonrpc') {
      return rpc_url;
    }
    if (key == 'TLE_exporter') {
      return enabled_exporter;
    }
  ...
  TLE.setConfig = function(key, value) {
    if (key == 'TLE_aria2_jsonrpc' || key == 'TLE_exporter') {
      return;
    }
    if (window.localStorage) {
  ...
  function init() {
    ...
    $("#setting_main_tpl").text($("#setting_main_tpl").text().replace(/(<\/div>\s+<div class="btnin">)/,
          '<div class="doline mag01" style="display:none;"></div>'
            +'<h3 style="background-position: 0 -180px;display:none;">Thunder Lixian Exporter 设定</h3>'
